...
function App() {
  ...
  const [maxCars, setMaxCars] = useState(0);
  const [pageClicked, setPageClicked] = useState(0);
  const [buttonClasses, setButtonClasses] = useState(`flex items-center justify-center w-[24px] h-[24px] rounded-full cursor-pointer ${isDollar ? "bg-black" : "bg-slate-200"} text-gray-800 ${isDollar ? "icon-white" : "icon-gray-800"}`)
  const [buttonClasses2, setButtonClasses2] = useState(`flex items-center justify-center w-[24px] h-[24px] rounded-full cursor-pointer ${isDollar ? "bg-slate-200":"bg-black"} text-gray-800 ${isDollar ? "icon-gray-800" : "icon-white"}`)
  const [periodTxt, setPeriodTxt] = useState("პერიოდი")
  const [sortTxt, setSortTxt] = useState("სორტირება")
  ...
  const ManDropDown: React.FC<{ manufacturer: ManufacturerListType[] }> = ({
    ...
    useEffect(()=>{
      const buttonClasses1 = `flex items-center justify-center w-[24px] h-[24px] rounded-full cursor-pointer ${isDollar ? "bg-black" : "bg-slate-200"} ${isDollar ? "text-white":"text-gray-800"} ${isDollar ? "icon-white" : "icon-gray-800"}`;
      const buttonClasses3 = `flex items-center justify-center w-[24px] h-[24px] rounded-full cursor-pointer ${isDollar ? "bg-slate-200":"bg-black"} ${isDollar ? "text-gray-800":"text-white"} ${isDollar ?  "icon-gray-800": "icon-white"}`
    ...
    return (
      <div className="relative">
        ...
        />
        {isOpen && (
          <div className="w-[198px] max-h-[324px] mt-[15px] overflow-y-auto absolute top-12 left-0 bg-white border rounded-md shadow-md z-50">
  ...
  const sortManfacurer = (manlst: ManufacturerListType[]) => {
    const sortedList = manlst.sort((a, b) =>
      a.man_name.localeCompare(b.man_name)
    );
    return sortedList;
  };
  ...
  const FilteringUrl = (indicator: number) => {
    ...
    if (isNaN(fromValue)) {
      ...
      if (isDollar){
        from = `${Math.round(fromValue*2.6089)}`;
      }else{
        from = `${fromValue}`;
      }
    }
  ...
  const fetchData1 = async () => {
    ...
    try {
      ...
      setMaxCars(responseData.data.meta.total);
    } catch (error) {
      setLoading(false);
      console.log(error);
    }
    ...
    if (downloaded.includes(toDownload) === false) {
      try {
        const response = await fetch(
          `https://api2.myauto.ge/ka/getManModels?man_id=${toDownload}`
        );
  ...
  const handlePeriodFilter = (period: string) => {
    switch (period) {
      case '1h':
        setPeriodTxt('ბოლო 1 საათი');
        break;
      case '2h':
        setPeriodTxt('ბოლო 2 საათი');
        break;
      case '3h':
        setPeriodTxt('ბოლო 3 საათი');
        break;
      case '1d':
        setPeriodTxt('ბოლო 1 დღე') ;
        break;
      case '2d':
        setPeriodTxt('ბოლო 2 დღე') ;
        break;
      case '3d':
        setPeriodTxt('ბოლო 3 დღე') ;
        break;
      case '1w':
        setPeriodTxt('ბოლო 1 კვირა') ;
        break;
      case '2w':
        setPeriodTxt('ბოლო 2 კვირა') ;
        break;
      case '3w':
        setPeriodTxt('ბოლო 3 კვირა') ;
        break;
      default:
        setPeriodTxt('პერიოდი') ;
    }
  ...
  const handleSortOption = (option: string) => {
    switch (option) {
      case '1':
        setSortTxt("თარიღი კლებადი");
        break;
      case '2':
        setSortTxt('თარიღი ზრდადი');
        break;
      case '3':
        setSortTxt('ფასი კლებადი');
        break;
      case '4':
        setSortTxt('ფასი ზრდადი') ;
        break;
      case '5':
        setSortTxt('გარბენი კლებადი') ;
        break;
      case '6':
        setSortTxt('გარბენი ზრდადი') ;
        break;
      default:
        setSortTxt('სორტირება') ;
    }
  ...
  const svgStyleButton1 = {
    fill: isDollar ? "white" : "#272a37",
  };
  const svgStyleButton2 = {
    fill: isDollar ? "#272a37" : "white",
  };

  let rentText: string = "გარიგების ტიპი";
  if (checkbox1Checked) {
    rentText = "იყიდება";
  ...
  const clearFilterRent = () => {
    let newState = rentFilter;
    newState["rent"] = 0;
    newState["sale"] = 0;
    setRentFilter(newState);
    rentText = "გარიგების ტიპი";
    setCheckbox1Checked(false);
    setCheckbox2Checked(false);
  };
  ...
  return (
    <div className="flex justify-center flex-col bg-gray-100">
      ...
      <div className="flex flex-no-wrap justify-center">
        <div className="flex flex-col mr-20 h-[385px] p-[20px]">
          {" "}
          {/* marjvena mxare */}
          <div className="flex flex-row items-center mb-[20px]">
            <div className="text-[12px] text-gray-500 mr-[10px]">მთავარი</div>
            ...
            <div className="text-[12px] text-gray-500 mr-[10px]">ძიება</div>
            ...
            <div className="text-custom">იყიდება</div>
          </div>
          <div>
          ...
          <div className="flex flex-col mr-20 h-[450px] w-[240px] p-[20px] bg-white">
            <div className="mb-[10px]">
              {" "}
              {/* garigebis tipebi */}
              <div className="text-[12px] font-medium">გარიგების ტიპი</div>
              <div className="relative inline-block">
                <button
                  className="w-[198px] text-start hover:border-gray-500 bg-white border text-gray-400 py-2 px-4 rounded"
                  type="button"
                ...
                {isOpenSale && (
                  <div className=" w-[198px] max-h-[324px] mt-[12px] absolute top-12 left-0 z-10 bg-white border rounded-md shadow-md">
                    <label
                      htmlFor="-1"
                      className="block p-2 text-gray-500 text-[14px] font-normal"
                    >
                      <input
                        type="checkbox"
                        id="-1"
                        ...
                        className="mr-2 rounded-[4px] w-[16px] h-[16px] accent-custom"
                      />
                      {"  იყიდება"}
                    </label>
                    <label
                    ...
                    <div>
                      {(rentFilter["rent"] == 1 || rentFilter["sale"] == 1) && (
                        <div className="flex flex-row items-center justify-center">
                          <div>
                            <hr></hr>
                            <button
                              title="clean the filter"
                              onClick={() => clearFilterRent()}
                              className="h-[50px] bg-white text-gray-400 text-[12px] py-2 px-4 rounded-md hover:text-black"
                            >
                              ფილტრის გასუფთავება
                            </button>
            ...
            <div className="mb-[10px]">
              {" "}
              {/* mwarmoebeli */}
              <div className="text-[12px] font-medium">მწარმოებელი</div>
              <div>
                <ManDropDown
                  manufacturer={manufacturer.filter((info) => {
                    if (vehicle === 0) {
            ...
            <div className="mb-[10px]">
              {" "}
              {/* kategoria */}
              <div className="text-[12px] font-medium">კატეგორია</div>
              <div>
                <CategoryDropDown
                  categories={categoty.filter(
                    (info) => info.vehicle_types[0] === vehicle
                  )}
                ></CategoryDropDown>
              </div>
            </div>
            <div className="mb-[4px]">
              {" "}
              {/* modelebi */}
              <div className="text-[12px] font-medium">მოდელი</div>
              <div>
                <ModelDropDown
                  models={modelsState.filter(
                    (model) =>
                      manufacturerPotential.hasOwnProperty(model.man_id) &&
                      manufacturerPotential[model.man_id] === 1
                  )}
                  updateModels={updateModels}
                  mans={manufacturerPotential}
                ></ModelDropDown>
              </div>
            </div>
            {/* shercheva fasis mixedvit */}
            <hr className="w-[240px] relative right-8"></hr>
            <div className="flex flex-row justify-between">
            <div className="text-[12px] font-medium">ფასი</div>
            <div className="flex flex-row w-[50px]">
            <button onClick={()=>{
            setIsDollar(false); console.log(isDollar)}}><div className={buttonClasses2}><svg xmlns="http://www.w3.org/2000/svg" width="10px" height="11px" viewBox="0 0 10 11"><path id="GEL" d="M313.914-18v-1.689h-3.663a2.938,2.938,0,0,1-1.643-.46,3,3,0,0,1-1.089-1.3,4.608,4.608,0,0,1-.384-1.94,5,5,0,0,1,.343-1.987,2.543,2.543,0,0,1,1.112-1.225v3.372h.894v-3.64a2.492,2.492,0,0,1,.48-.044,2.936,2.936,0,0,1,.5.044v3.64h.894V-26.6a2.469,2.469,0,0,1,1.134,1.24,5.547,5.547,0,0,1,.343,2.132H315a6.022,6.022,0,0,0-.439-2.324,4.874,4.874,0,0,0-1.263-1.8,4.534,4.534,0,0,0-1.939-1.019V-29h-.894v.472l-.236-.007q-.081-.007-.236-.007-.347,0-.51.015V-29h-.894v.631a4.67,4.67,0,0,0-1.891.982,4.823,4.823,0,0,0-1.256,1.671A4.872,4.872,0,0,0,305-23.67a5.7,5.7,0,0,0,.229,1.61,4.62,4.62,0,0,0,.672,1.4,3.294,3.294,0,0,0,1.056.968v.058h-1.411V-18Z" transform="translate(-305 29)" style={svgStyleButton2}></path></svg></div></button>
              <button onClick={()=>{
            setIsDollar(true); console.log(isDollar)}}><div className={buttonClasses}><svg xmlns="http://www.w3.org/2000/svg" width="9px" height="14px" viewBox="0 0 9 14"><path id="_" data-name="$" d="M1.974-3.248H0A3.8,3.8,0,0,0,1.022-.742a3.783,3.783,0,0,0,2.45.966v1.19H4.3V.224A4.359,4.359,0,0,0,6.02-.2a3.707,3.707,0,0,0,1.071-.8,2.62,2.62,0,0,0,.553-.917A2.6,2.6,0,0,0,7.8-2.688a7.425,7.425,0,0,0-.049-.8,2.226,2.226,0,0,0-.315-.9,3.024,3.024,0,0,0-.826-.861,4.839,4.839,0,0,0-1.6-.693q-.2-.056-.371-.1L4.3-6.118V-8.6A1.033,1.033,0,0,1,5.11-8.2a1.472,1.472,0,0,1,.35.952H7.448a3.209,3.209,0,0,0-.308-1.26A2.783,2.783,0,0,0,6.454-9.4a3.178,3.178,0,0,0-.973-.56A5.033,5.033,0,0,0,4.3-10.234v-1.078H3.472v1.078a4.667,4.667,0,0,0-1.218.245,3.291,3.291,0,0,0-1.036.574A2.8,2.8,0,0,0,.5-8.5a2.782,2.782,0,0,0-.273,1.26A2.569,2.569,0,0,0,.462-6.069a2.325,2.325,0,0,0,.637.784,3.337,3.337,0,0,0,.9.5q.5.189,1.022.329.14.028.259.063t.189.063V-1.4a1.955,1.955,0,0,1-1.078-.588A1.72,1.72,0,0,1,1.974-3.248ZM4.3-1.4V-4.088a3.381,3.381,0,0,1,1.169.5.983.983,0,0,1,.343.819,1.152,1.152,0,0,1-.14.581,1.385,1.385,0,0,1-.357.413,1.641,1.641,0,0,1-.49.259A2.555,2.555,0,0,1,4.3-1.4ZM3.472-8.6v2.282a2.3,2.3,0,0,1-.966-.406.889.889,0,0,1-.294-.714,1.162,1.162,0,0,1,.1-.511A1.048,1.048,0,0,1,2.6-8.309a1.219,1.219,0,0,1,.406-.217A1.54,1.54,0,0,1,3.472-8.6Z" transform="translate(0.544 11.812)" style={svgStyleButton1} stroke="rgba(0,0,0,0)" stroke-width="1"></path></svg></div></button>
            </div>
            </div>
            <div className="flex flex-row items-center justify-center mt-[10px]">
              <input
                ...
                id="number1"
                placeholder="  დან"
                className="border w-[90px] h-[32px] mr-[7px] rounded-[6px]"
                value={fromValue}
              ...
              <input
                ...
                id="number2"
                placeholder="  მდე"
                className="border w-[90px] h-[32px] ml-[7px] rounded-[6px]"
                value={toValue}
        ...
        <div className="flex justify-baseline  flex-col  ">
          ...
          {/* marcxena mxare */}
          <div className="flex flex-row justify-between">
            <div className="text-[15px]">{`${maxCars} განცხადება`}</div>
            {/* periodi da daxarisxeba */}
            <div>
              <PeriodDropDown handlePeriodFilter={handlePeriodFilter} handleSortOption={handleSortOption} periodTxt={periodTxt} sortTxt={sortTxt}></PeriodDropDown>
            </div>
          </div>
          <div>
            {" "}
            {/* manqanebi */}
            {data.length == 0 ? (<div className="w-[750px] h-[550px] mt-[50px] text-center font-bold text-5xl">განცხადებები ვერ მოიძებნა<div className="mt-[50px] text-center text-2xl">შენი ძებნის პარამეტრების მიხედვით განცხადებები ვერ მოიძებნა. შეცვალე ან გამოიწერე პარამეტრები და მიიღე შეტყობინება ახალი განცხადებების განთავსების შემთხვევაში</div></div>):(<></>)}
            {data.map((info) => {
              return <CarInfo key={info.car_id} {...info} isDollar={isDollar}/>;
            })}
          </div>
          {data.length == 0 ?(<></>) :(
